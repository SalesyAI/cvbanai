...
async function sendEmail(options) {
    ...
    try {
        ...
        if (error) {
            console.error('[BetterAuth] Resend API Error:', error);
        } else {
            console.log('[BetterAuth] Resend Success:', data);
        }
    } catch (error) {
        console.error('[BetterAuth] Email send exception:', error.message);
    }
}
...
export const auth = betterAuth({
    ...
    emailAndPassword: {
        enabled: true,
        requireEmailVerification: true, // ENFORCED: User must verify email to login
    ...
    // Hooks for Google Sheets & Discord Sync
    databaseHooks: {
        user: {
            create: {
                after: async (user) => {
                    // Trigger the on-signup webhook
                    // If running locally, target the backend server directly (port 3001) instead of the frontend (port 5173)
                    // This avoids dependency on the frontend server being running/proxying during auth operations
                    const serverPort = process.env.PORT || 3001;
                    const webhookBase = siteUrl.includes('localhost')
                        ? `http://localhost:${serverPort}`
                        : siteUrl;

                    const webhookUrl = `${webhookBase}/api/webhooks/on-signup`;
                    console.log(`[BetterAuth Hook] Triggering webhook: ${webhookUrl}`);

                    try {
                        // We use fetch directly since BetterAuth runs in Node context
                        ...
                        console.log('[BetterAuth Hook] Webhook triggered successfully');
                    } catch (err) {
                        console.error('[BetterAuth Hook] Failed to trigger webhook:', err.message);
                    }
                }
            }
        }
